...
public class multiply extends ICBuiltinOperator
{
	public multiply(ICCalculatorBase calc)
	{
		super(calc);
		operatorSymbol ='*';
	}
	...
	public boolean eval(ICLinkedList output, Node position, String basis, double parentIn)
	{
		if(position.prev!=null && position.prev.data.type==ICMathToken.Types.NUMBER) //if the last token exists, and is a number
		{
			...
			{
		position.prev.prev.data.number=position.prev.prev.data.number*position.prev.data.number; //change the number of the first token in the operation
