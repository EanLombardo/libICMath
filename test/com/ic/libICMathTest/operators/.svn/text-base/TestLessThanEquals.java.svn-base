...
public class TestLessThanEquals
{
	...
	public double lessEquals(double one, double two)
	{
		if(one<=two)
		{
			return 1.0D;
	...
	{
		assertEquals(calc.stringEvalExpression("0≤0"),nts(1.0D));
	}
	@Test
	public final void oneZero()
	{
		assertEquals(calc.stringEvalExpression("0≤4.293781632"),nts(1.0D));
	}
	@Test
	public final void noZero()
	{
		assertEquals(calc.stringEvalExpression("239193.193948719782≤4.293781632"),nts(0.0D));
	}
	@Test
	public final void atonOfRandoms()
	{
		...
		for(int i=0;i<OperatorTests.randoms;i++)
		{
			...
			assertEquals(calc.stringEvalExpression(nts(one)+"≤"+nts(two)),nts(lessEquals(one,two)));
		}
	}
